...
def test_system(example_system):
    ...
    assert np.array_equal(example_system.positions.magnitude, np.array([[0.0, 0.0, 0.0], [0.1, 0.1, 0.1]]))
...
def test_bravais_lattice(example_bravais_lattice):
    ...
    assert np.array_equal(example_bravais_lattice.points.magnitude, np.array([[0.0, 0.0, 0.0], [0.1, 0.1, 0.1], [0.2, 0.2, 0.2]]))
...
def test_model(example_model):
    ...
    assert np.array_equal(example_model.onsite_energies.magnitude, np.array([0.5, 0.6, 0.7, 0.8]))
